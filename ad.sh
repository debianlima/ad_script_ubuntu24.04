...
configure_bind() {
    ...
    sudo samba-tool dns zonecreate ${SERVER} ${DOMAIN}
    sudo samba-tool dns add ${SERVER} ${DOMAIN} ${SERVER} A ${IP}
...
EOF
    ...
    echo "Identificar a versão do BIND9"
    ...
    # Configurar named.conf.local para adicionar o include
    sudo bash -c "echo 'include \"/var/lib/samba/bind-dns/named.conf\";' >> ${BIND_CONF_DIR}/named.conf.local"

    if [ $? -eq 0 ]; then
        echo "BIND9 configurado e iniciado com sucesso."
    else
        echo "Falha ao configurar e iniciar o BIND9."
    fi
